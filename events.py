...
def _event(
    ...
    *,
    ...
    return {
        ...
        "required_traits": list(required_traits) if required_traits is not None else [],
    }
...
HUMAN_MONTHLY_EVENTS = [
    ...
    _event("infant_sleep_grow", "You spend the month sleeping and eating, growing stronger.", life_stages=["Infant"], max_age_months=12, outcome={"stat_changes": {"health": 1, "strength": 1}}, tags=["positive", "infant"]),
    ...
    _event("child_read_book", "You read a new children's book.", life_stages=["Child"], min_age_months=48, outcome={"stat_changes": {"intelligence": 1, "wisdom": 1}}, tags=["neutral", "child"]),
    ...
    _event("child_playdate", "You had a fun playdate with another child.", life_stages=["Child"], min_age_months=48, outcome={"stat_changes": {"happiness": 2, "charisma": 1}}, tags=["positive", "child"]),
    ...
    _event("teen_join_club", "You join an after-school club and find it surprisingly rewarding.", life_stages=["Teenager"], outcome={"stat_changes": {"happiness": 1, "charisma": 1}}, tags=["positive", "teenager"]),
    _event("teen_study_session", "You spend extra time studying and understand a difficult topic a little better.", life_stages=["Teenager"], outcome={"stat_changes": {"intelligence": 1, "discipline": 1}}, tags=["neutral", "teenager"]),
    _event("teen_pickup_game", "You join a casual game with other teens and come home tired but upbeat.", life_stages=["Teenager"], min_age_months=132, outcome={"stat_changes": {"health": 1, "strength": 1}}, tags=["positive", "teenager"]),
    ...
    _event("teen_new_style", "You tried a new look and felt unexpectedly confident about it.", life_stages=["Teenager"], outcome={"stat_changes": {"happiness": 1, "looks": 1}}, tags=["positive", "teenager"]),
    ...
    _event("teen_long_thought", "You stayed up late thinking about who you want to become.", life_stages=["Teenager"], outcome={"stat_changes": {"intelligence": 1, "wisdom": 1}}, tags=["neutral", "teenager"]),
    ...
    _event("young_adult_library_visit", "You spend an afternoon at the library and leave with a fresh idea.", life_stages=["Young Adult"], outcome={"stat_changes": {"intelligence": 1, "wisdom": 1}}, tags=["positive", "young_adult"]),
    ...
    _event("adult_nice_conversation", "A brief conversation leaves you in a better mood than expected.", life_stages=["Adult"], outcome={"stat_changes": {"happiness": 1, "charisma": 1}}, tags=["positive", "adult"]),
    ...
    _event("adult_evening_stretch", "You gave your body a little care and felt the difference.", life_stages=["Adult", "Elder"], outcome={"stat_changes": {"health": 1, "looks": 1}}, tags=["positive"]),
...
def _get_human_eligible_events(lifecycle_state: dict, family_context=None, actor_traits=None) -> list[dict]:
    ...
    actor_traits = set(actor_traits or [])
    ...
    for event in HUMAN_MONTHLY_EVENTS:
        ...
        required_traits = event.get("required_traits") or []
        if required_traits and actor_traits.isdisjoint(required_traits):
            continue
