...
Use source documents in this priority order:
	...
	6.	Actionable Summary, if one is actively maintained
...
Interpretation rule:
	...
	•	Actora Roadmap defines sequencing and dependency order
	...
	•	Actionable Summary is a temporary planning aid, not durable authority
...
Use documents like this:
	...
	•	Actora Roadmap for sequencing and dependency order, not patch-by-patch progress tracking
	•	[[source-index|Source Index]] for source navigation and precedence
	•	Working Ideas Register for active idea intake and staging
	•	Actionable Summary for temporary planning snapshots when useful
...
Do not:
	...
	•	use the Actionable Summary as durable authority
	•	use the Changelog for speculative planning
	•	make the Actora Roadmap depend on current patch numbers if dependency order is all that matters
...
Use the Actionable Summary, if maintained, as a temporary planning snapshot rather than durable truth.
