...
### World (`world.py`)
The `World` class stores shared simulation state, including world-owned structured records.
...
4. Display the initial player snapshot.
5. Enter the game loop (`game_loop(...)`).
6. Read player input.
7. Resolve months to advance (or quit).
8. Call `World.simulate_advance_turn(...)`.
9. Advance time internally month-by-month.
10. Apply triggered event outcomes through `World.apply_outcome(...)`.
11. Collect triggered structured events.
12. Render updated snapshot.
13. Render event output by combining structured `year`, `month`, and raw event `text`.
...
- statistics (health, happiness, intelligence, money)
- family references (mother, father), still resolved from the world layer without visible terminal changes
