...
from views.profile import (
    build_person_card_lines,
    build_profile_popup_render_data,
    ...
    def handle_profile_key(self, key):
        if self.profile_popup_open:
            ...
            popup_data = build_profile_popup_render_data(
                self.profile_popup_category or PROFILE_CATEGORIES[self.profile_selected_row],
                self.get_snapshot_data(),
                getattr(self, "_profile_content_width", 88),
                self.profile_body_height,
            )
            if key in (curses.KEY_UP, ord("w"), ord("W")):
